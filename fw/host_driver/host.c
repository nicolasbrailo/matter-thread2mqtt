...
// and the BT channel is bridged to a kernel virtual HCI controller via /dev/vhci,
// so BlueZ sees a new hciN adapter backed by the ESP32 controller. A PING is sent
// once a second for link liveness. Run as root (needs CAP_NET_ADMIN for /dev/vhci);
// without it the BT channel is hex-dumped instead and the rest still works.
//
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        pfds[1].fd = g_vhci_fd; pfds[1].events = POLLIN; pfds[1].revents = 0;
        const int nfds = (g_vhci_fd >= 0) ? 2 : 1;
        ...
        // out to /dev/vhci inside process_frames).
